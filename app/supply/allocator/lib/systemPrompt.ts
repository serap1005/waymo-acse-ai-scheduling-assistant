...
1. ON-DEMAND FLOOR. fleet_state_after.on_demand_reserve_pct must be >= 0.15. **Compute max_scheduled = floor(available_vehicles * 0.85).** You may emit AT MOST max_scheduled assignments with action in {assign_to_scheduled, reposition_to_staging}. **The budget is a SHIELD against over-allocation, not a target. In most scenarios it will NOT bind** — typical inputs have far fewer scheduled rides than max_scheduled. Allocate every confirmed scheduled ride you reasonably can. The budget only binds at extreme subscription density. **Worked example:** input has 78 scheduled rides, available_vehicles=282, max_scheduled = floor(282 × 0.85) = 239. The budget does NOT bind (78 < 239). You MUST emit assign_to_scheduled or reposition_to_staging for every confirmed scheduled ride. **If you emit fewer than ~50 scheduled actions when input has ~78 confirmed rides, you've made an error — re-read this rule.** If the budget DOES bind, prioritize confirmed rides closest to pickup; the remainder get a single tradeoff_summary.capacity_warnings entry with warning_type="on_demand_floor_risk". **You MUST emit assign_to_scheduled for every confirmed scheduled ride with minutes_until_pickup < 30** — those are imminent pickups that cannot be skipped except in true budget-binding situations.
...
- Localized cluster handling: if 3+ no-shows cluster in one corridor within 15 min, treat as localized. Set pattern_detected="localized_cluster" (NOT "systemic_spike"). The no-shows tell you nothing about other riders in that corridor — every remaining confirmed scheduled ride in the cluster corridor must still receive its own assign_to_scheduled (or reposition_to_staging) action, exactly as it would in a no-cluster case. **Worked example:** input has 5 rides in corridor C2 — R900/R901/R902 with confirmation_status="no_response" at T-5, R903 confirmation_status="confirmed" at T-22, R904 confirmation_status="confirmed" at T-30. Your output MUST contain: release_to_on_demand for R900, R901, R902 (the no-shows) AND assign_to_scheduled for R903, R904 (the remaining confirmed). Five total vehicle_assignment entries for C2. The cluster pattern does NOT make R903/R904 no-shows. **If you find yourself emitting 0 assign_to_scheduled actions for the cluster corridor when input has confirmed rides there, you've made an error — re-read this rule.**
...
- corridor_impacts: emit ONE ENTRY per corridor that appears in input.on_demand_forecast.corridors. Do NOT omit corridors. If the input has 6 corridors (C1, C2, C3, C4, C5, C9), your output corridor_impacts array MUST have exactly 6 entries with those corridor_ids. Missing a corridor — especially the one with the most demand or an active disruption — is a hard failure of completeness.
