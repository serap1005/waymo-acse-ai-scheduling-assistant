      ...
      >
        <span
          style={{
            ...
            borderRadius: "50%",
            background: "#22D3EE",
            animation: "eval-pulse 0.8s ease-in-out infinite",
          }}
        />
        RUNNING
      ...
      >
        <span title={caseDef.critical ? "Critical (must pass for launch)" : ""}>
          <span
            style={{
              fontSize: 11,
              color: caseDef.critical ? "#F59E0B" : "#1E293B",
              fontWeight: 800,
            }}
          >
            ★
        ...
        <button
          ...
          style={{
            padding: "5px 11px",
            borderRadius: 999,
            background: disabled || isRunning ? "#1E293B" : "transparent",
            color: disabled || isRunning ? "#64748B" : "#22D3EE",
            border: `1px solid ${disabled || isRunning ? "#1E293B" : "#22D3EE66"}`,
            fontSize: 10,
            fontWeight: 700,
            letterSpacing: "0.1em",
          ...
          {result && !result.error && (
            <>
              ...
              <div style={{ fontSize: 10, color: "#475569", display: "flex", gap: 16 }}>
                ...
                <span>
                  Tokens: {result.tokens.input + result.tokens.cached_input + result.tokens.cache_creation_input} in
                  / {result.tokens.output} out
                  {result.tokens.cached_input > 0 && (
                    <span style={{ color: "#22C55E", marginLeft: 4 }}>
                      ({result.tokens.cached_input} cached)
                    </span>
                  )}
                </span>
              </div>
