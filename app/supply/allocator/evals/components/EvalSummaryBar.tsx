...
import type { EvalCase, EvalResult } from "../../lib/schemas";
...
export function EvalSummaryBar({ cases, results, runAllActive, onRunAll, onClear }: Props) {
  ...
  const launchGateLabel =
    runCount < total
      ? `${runCount}/${total} run`
      : launchGateMet
        ? "PASS"
        : "FAIL";
  const launchGateColor =
    runCount < total ? "#64748B" : launchGateMet ? "#22C55E" : "#EF4444";
  ...
  return (
    ...
    >
      <div style={{ display: "flex", gap: 28, flexWrap: "wrap", alignItems: "baseline" }}>
        <Metric
          label="Launch gate"
          value={launchGateLabel}
          color={launchGateColor}
          hint={`>=${(LAUNCH_GATE_OVERALL * 100).toFixed(0)}% overall + 100% critical`}
        />
        <Metric
          label="Passed"
          value={`${passed} / ${runCount || total}`}
          color={runCount === 0 ? "#94A3B8" : passed === runCount ? "#22C55E" : "#F59E0B"}
          hint={runCount > 0 ? `${(overallPct * 100).toFixed(0)}%` : "not yet run"}
        />
        <Metric
          label="Critical"
          value={`${criticalPassed} / ${criticalTotal}`}
          color={
        ...
        <Metric
          label="Avg latency"
          value={runCount > 0 ? `${avgLatency.toFixed(2)}s` : "—"}
          color="#E2E8F0"
          hint={runCount > 0 ? `${runCount} run` : ""}
        />
      </div>
      ...
      <div style={{ display: "flex", gap: 8 }}>
        <button
          ...
          style={{
            padding: "10px 18px",
            borderRadius: 10,
            background: runAllActive ? "#1E293B" : "#22D3EE",
            color: runAllActive ? "#64748B" : "#0B1020",
            border: "none",
            fontSize: 12,
            fontWeight: 700,
            letterSpacing: "0.05em",
            cursor: runAllActive ? "wait" : "pointer",
          }}
        >
    ...
    return (
      <div>
        ...
        <div style={{ display: "flex", alignItems: "baseline", gap: 6, marginTop: 2 }}>
          <span style={{ fontSize: 20, color, fontWeight: 800, fontVariantNumeric: "tabular-nums" }}>
            {value}
          </span>
