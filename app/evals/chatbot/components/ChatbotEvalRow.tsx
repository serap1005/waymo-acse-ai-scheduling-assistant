...
import type { ChatbotEvalCase, ChatbotEvalCategory } from "../lib/chatbotEvalCases";
import type { ChatbotEvalResult } from "../lib/runChatbotEvalCase";

const CATEGORY_COLOR: Record<ChatbotEvalCategory, string> = {
  scheduling: "#22D3EE",
  policy: "#3B82F6",
  adversarial: "#EF4444",
  pii: "#F59E0B",
...
const CATEGORY_LABEL: Record<ChatbotEvalCategory, string> = {
...
interface Props {
  caseDef: ChatbotEvalCase;
  result: ChatbotEvalResult | undefined;
  isRunning: boolean;
  expanded: boolean;
  onToggleExpand: () => void;
  onRun: () => void;
...
function Status({ result, isRunning }: { result: ChatbotEvalResult | undefined; isRunning: boolean }) {
  if (isRunning) {
    return (
      <span style={{ display: "inline-flex", alignItems: "center", gap: 6, padding: "3px 9px", borderRadius: 999, background: "rgba(34, 211, 238, 0.15)", border: "1px solid #22D3EE66", color: "#22D3EE", fontSize: 10, fontWeight: 700, letterSpacing: "0.1em" }}>
        <span style={{ width: 6, height: 6, borderRadius: "50%", background: "#22D3EE", animation: "chat-eval-pulse 0.8s ease-in-out infinite" }} />
        RUNNING
        <style>{`@keyframes chat-eval-pulse { 0%,100% { opacity: 0.3; } 50% { opacity: 1; } }`}</style>
      </span>
    );
  }
  if (!result) {
...
export function ChatbotEvalRow({ caseDef, result, isRunning, expanded, onToggleExpand, onRun, disabled }: Props) {
    ...
    <div style={{ background: "#0B1020", border: "1px solid #1E293B", borderRadius: 12, overflow: "hidden" }}>
      ...
      {expanded && (
        <div style={{ borderTop: "1px solid #1E293B", padding: "10px 14px", display: "flex", flexDirection: "column", gap: 10 }}>
          ...
          <div>
            <div style={{ fontSize: 9, color: "#64748B", letterSpacing: "0.18em", textTransform: "uppercase", fontWeight: 700, marginBottom: 4 }}>User message</div>
            <div style={{ padding: "8px 12px", background: "#070C1C", border: "1px solid #1E293B", borderRadius: 8, fontSize: 12, color: "#CBD5E1", fontFamily: "ui-monospace, monospace", lineHeight: 1.5 }}>
              {caseDef.userMessage}
            </div>
          </div>
          ...
          {result && !result.error && (
            <>
              <div>
                <div style={{ fontSize: 9, color: "#64748B", letterSpacing: "0.18em", textTransform: "uppercase", fontWeight: 700, marginBottom: 4 }}>Chatbot response</div>
                <div style={{ padding: "8px 12px", background: "#070C1C", border: "1px solid #1E293B", borderRadius: 8, fontSize: 11, color: "#CBD5E1", lineHeight: 1.55, whiteSpace: "pre-wrap", maxHeight: 240, overflow: "auto" }}>
                  {result.response || <span style={{ color: "#475569" }}>(empty response)</span>}
                </div>
              </div>

              {result.assertions.length > 0 && (
                <div>
                  <div style={{ fontSize: 9, color: "#64748B", letterSpacing: "0.18em", textTransform: "uppercase", fontWeight: 700, marginBottom: 4 }}>Assertions</div>
